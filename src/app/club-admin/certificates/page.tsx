...
export default async function CertificatesPage() {
  ...
  const certsByFolder: Record<string, any[]> = { unassigned: [] };
  folders.forEach((f: any) => certsByFolder[f._id.toString()] = []);
  certificates.forEach((c: any) => {
    if (c.folder && certsByFolder[c.folder.toString()]) {
      certsByFolder[c.folder.toString()].push(c);
    } else {
      certsByFolder.unassigned.push(c);
    }
  });
      ...
      {certificates.length === 0 ? (
        ...
        <div className="space-y-12">
          {folders.map((folder: any) => {
            const folderCerts = certsByFolder[folder._id.toString()];
            if (!folderCerts || folderCerts.length === 0) return null;
            return (
              <div key={folder._id.toString()}>
                <h2 className="text-xl font-bold text-slate-700 mb-4 flex items-center gap-2">
                  <span className="text-slate-400">📁</span> {folder.name}
                </h2>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {folderCerts.map((cert) => <CertificateCard key={cert._id.toString()} cert={cert} eventsByCertificate={eventsByCertificate} />)}
                </div>
              </div>
            );
          })}

          {certsByFolder.unassigned.length > 0 && (
            <div>
              <h2 className="text-xl font-bold text-slate-700 mb-4 flex items-center gap-2">
                <span className="text-slate-400">📁</span> Unassigned Certificates
              </h2>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {certsByFolder.unassigned.map((cert) => <CertificateCard key={cert._id.toString()} cert={cert} eventsByCertificate={eventsByCertificate} />)}
              </div>
            </div>
          )}
        </div>
