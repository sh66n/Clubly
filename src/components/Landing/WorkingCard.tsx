...
export function WorkingCard({ children }: WorkingCardProps) {
  ...
  return (
    <div className="group relative h-80 w-80 rounded-full text-center border border-[#515151] overflow-hidden transition-colors duration-700 hover:border-transparent">
      {/* Background overlay */}
...
export function Number({ children }: { children: React.ReactNode }) {
  return (
    <div className="heading text-xl text-[#5E77F5] transition-all duration-300 group-hover:text-white">
      {children}
    </div>
  );
}
...
export function Working({ children }: { children: React.ReactNode }) {
  return (
    <div className="text-lg transition-all duration-300 group-hover:text-white">
      {children}
    </div>
  );
}
